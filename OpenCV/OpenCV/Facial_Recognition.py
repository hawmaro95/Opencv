...
while True:
    ...
    if (ret0):
        ...
        for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
            ...
            if matches[matchIndex]:
                ...
                if CheckIn[id] != 0:
                    #id = 2
                    AttendanceOut(name,CheckIn[id],id)
                    CheckIn[id] = 0
    ...
    if (ret1):
        ...
        for encodeFace2,faceLoc2 in zip(encodesCurFrame2,facesCurFrame2):
            matches2 = face_recognition.compare_faces(encodeListKnown,encodeFace2)
            faceDis2 = face_recognition.face_distance(encodeListKnown,encodeFace2)
            matchIndex2 = np.argmin(faceDis2)
            R = 255
            G = 0
            if matches2[matchIndex2]:
                name2 = classNames[matchIndex2]
                R = 0
                G = 255
                ...
                if CheckIn[id] == 0:
                    n = n+1
                    #print(name2)
                    Attendance(name2,n,id)
                    CheckIn[id] = n
            else: name2 = 'Unknown'
            y1,x2,y2,x1 = faceLoc2
            y1, x2, y2, x1 = y1*4,x2*4,y2*4,x1*4
